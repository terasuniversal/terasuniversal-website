...
| `/admin/courses` `/new` `/[id]` `/[id]/preview` | editor | ✅ complete — **full CRUD + preview** |
| `/admin/schedules` `/new` `/[id]` | editor | ✅ complete — **full CRUD + duplicate + cancel** |
| `/admin/participants` `/new` `/[id]` | editor | ✅ complete — **full CRUD + register to schedule** |
| `/admin/certificates` `/new` `/[id]` | editor | ✅ complete — **issue / revoke / reinstate + QR + verify link** |
...
tested; the CRUD screen replicates the fully-built Courses/Schedules/
Certificates modules (three files: `page.tsx`, `[id]/page.tsx`+`new/page.tsx`,
`actions.ts`). See §8.

**Complete training-operations loop now built end to end:** create a course →
schedule a session (duplicate/cancel supported) → register participants → mark
attendance & record assessment → issue a certificate (auto verification code +
QR) → anyone verifies it publicly at `/verify`.
...
- **Training Schedule** — ✅ built (full CRUD + duplicate + cancel).
- **Participants** — ✅ built (CRUD + register-against-schedule; feeds Attendance).
- **Certificates** — ✅ built (issue with auto verification code, revoke/
  reinstate, QR + public verify link).
- **News / Gallery / FAQ / Downloads / Company / Media** — remaining: standard
  CRUD; wire uploads to Supabase Storage via the `media` table.
