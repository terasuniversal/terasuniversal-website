...
export function CertificateDocument({ data, config }: { data: CertData; config: TemplateConfig }) {
  ...
  const isWorkingAtHeight = config.design_variant === "working_at_height_certificate" || /working\s+at\s+height/i.test(data.course_name || "");
  ...
  const showDurationRibbon = config.design_variant !== "standard_scaffold_certificate" && !isWorkingAtHeight;
