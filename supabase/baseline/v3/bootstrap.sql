-- ============================================================================
-- TERAS UNIVERSAL - Database Baseline V3 BOOTSTRAP
-- ----------------------------------------------------------------------------
-- NEW-ENVIRONMENT-ONLY. DO NOT RUN AGAINST PRODUCTION OR ANY POPULATED DB.
-- Schema-only bootstrap captured from Production and normalized through
-- canonical migration 20260912005428.
-- ============================================================================
BEGIN;
...
BEGIN
  ...
  IF marker IS NOT NULL THEN
    RAISE EXCEPTION 'baseline_marker_exists: app.app_schema_baseline already exists -- refusing Baseline V3' USING ERRCODE = 'P0001';
  END IF;
  existing := (SELECT array_agg(obj_name) FROM (
    SELECT unnest(ARRAY[
      'public.profiles','public.courses','public.participants',
      'public.sales_lead_metadata','public.sales_opportunities',
      'public.sales_quotations','public.invoices',
      'public.hrdf_claims','public.staff_module_catalog'
    ]) AS obj_name
  ) q WHERE to_regclass(obj_name) IS NOT NULL);
  IF existing IS NOT NULL AND cardinality(existing) > 0 THEN
    RAISE EXCEPTION 'non_empty_database: TERAS application objects already exist: % -- refusing Baseline V3 bootstrap', existing USING ERRCODE = 'P0001';
  END IF;
END
$$;
-- ============================================================================
-- TERAS UNIVERSAL - Database Baseline V3 canonical schema snapshot
-- ----------------------------------------------------------------------------
-- READ-ONLY VERIFIER. Do not run after bootstrap.sql.
-- Source: Production live schema, project iagzkrzeuawaxvacqprk; normalized with canonical migrations through 20260912005428.
...
-- Canonical migration DDL incorporated: 20260911090000_internal_sales_lead_creation.sql
...
-- Canonical migration DDL incorporated: 20260911115407_invoice_phase5_quotation_snapshots.sql
...
-- Canonical migration DDL incorporated: 20260911133000_invoice_phase5_snapshot_column_alignment.sql
-- Invoice Phase 5: align persisted invoice snapshot columns with the
-- canonical snapshot vocabulary. Existing values are preserved by renaming.

alter table public.invoices
  rename column quotation_no to quotation_number_snapshot;

alter table public.invoices
  rename column training_service_address to training_service_address_snapshot;

alter table public.invoice_items
  rename column course_id to course_id_snapshot;

comment on column public.invoices.quotation_number_snapshot is
  'Quotation number copied at invoice creation; display provenance, never a live join.';
comment on column public.invoices.training_service_address_snapshot is
  'Training/service destination copied from the accepted quotation; never used as billing_address.';
comment on column public.invoice_items.course_id_snapshot is
  'Course identity copied from the accepted quotation item at invoice creation.';
...
-- Canonical migration DDL incorporated: 20260911144803_hrdf_claim_tracking.sql
...
-- Baseline V3 excludes the HRDF module-catalog seed row; reference data is not captured.
...
-- Canonical migration DDL incorporated: 20260912005428_sst_readiness_invoice_corporate.sql
...
VALUES ('v3', '5A7F123069D41A62966E0007FF9E93B593FB72060AAE389E8CA9A913A8B660EF', 'TERAS Baseline V3 bootstrap complete from schema-only Production capture; no business rows, Auth rows, Storage objects, or secrets.')
