-- ============================================================================
-- TERAS UNIVERSAL - Database Baseline V1 BOOTSTRAP
-- ----------------------------------------------------------------------------
-- NEW-ENVIRONMENT-ONLY. DO NOT RUN AGAINST PRODUCTION OR ANY POPULATED DATABASE.
--
-- Purpose:
--   Reconstruct the canonical Baseline V1 application schema (public + app)
--   on a BRAND-NEW EMPTY Supabase/Postgres environment.
--
-- Guards (fail-fast, raise exception):
--   1. app.app_schema_baseline marker exists        -> already bootstrapped
--   2. any TERAS application table already exists   -> populated/production-like DB
--
-- On success the app.app_schema_baseline marker is created/inserted at the END.
-- A partial failure leaves the database dirty; do NOT re-run against it -
-- recreate a fresh empty environment and re-run there.
--
-- Platform schemas (auth, storage, realtime, vault, supabase_migrations) are
-- NOT created here; they are Supabase-managed and must already exist.
-- Native Supabase Auth is REQUIRED (app.handle_new_user / on_auth_user_created).
--
-- Source snapshot: schema.sql (SHA-256 D6E90DD4CB4E8F6E4B98EAC2AACAA49BA662E16929C4D21DD9282B6326C8B497), captured 2026-08-16 from
-- production project iagzkrzeuawaxvacqprk (Postgres 17.6).
-- ============================================================================


-- ----------------------------------------------------------------------------
-- GUARD 1 + 2: refuse to run on an already-bootstrapped or populated database.
-- ----------------------------------------------------------------------------
DO $$
DECLARE
  marker regclass;
...
BEGIN
  ...
  IF marker IS NOT NULL THEN
    RAISE EXCEPTION
      'baseline_marker_exists: app.app_schema_baseline already exists -- this database was already bootstrapped with TERAS Baseline V1. Refusing to run (NEW-ENVIRONMENT-ONLY).'
      USING ERRCODE = 'P0001';
  END IF;

  existing := (SELECT array_agg(obj_name) FROM (SELECT unnest(ARRAY[
      'public.profiles','public.courses','public.certificates','public.participants',
      'public.enquiries','public.proposal_requests','public.sales_lead_metadata',
      'public.sales_opportunities','public.sales_quotations','public.sales_activity',
      'public.sales_tasks','public.audit_logs','public.course_schedules',
      'public.attendance','public.assessments','public.media','public.news_posts',
      'public.faqs','public.gallery_images','public.downloads','public.company_profile',
      'public.photos','public.participant_feedback','public.admin_users'
    ]) AS obj_name) q WHERE to_regclass(obj_name) IS NOT NULL);

  IF existing IS NOT NULL AND cardinality(existing) > 0 THEN
    RAISE EXCEPTION
      'non_empty_database: this database already contains TERAS application tables: % -- the baseline bootstrap is NEW-ENVIRONMENT-ONLY and refuses to run against an existing populated or production-like database.',
...
-- ----------------------------------------------------------------------------
-- MARKER (created only after the schema above applied successfully).
-- ----------------------------------------------------------------------------
...
VALUES ('v1', 'D6E90DD4CB4E8F6E4B98EAC2AACAA49BA662E16929C4D21DD9282B6326C8B497', 'TERAS Baseline V1 bootstrap complete (new environment). Production schema snapshot 2026-08-16 (project iagzkrzeuawaxvacqprk).')
