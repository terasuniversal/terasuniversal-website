-- Certificate eligibility engine (single source of truth for
-- generateCertificate/bulkGenerate and the Generate Certificates UI).
--
-- security_invoker = true is required, not cosmetic: this view's owner
-- (postgres) has BYPASSRLS live (confirmed 2026-08-09), so without this
-- option the view would silently ignore every RLS policy on
-- courses/participants/certificates/course_schedules/schedule_participants/
-- attendance/assessments for whoever queries it -- the exact "RLS is the
-- real boundary, not app guards" failure mode CLAUDE.md §6/§12 warns about.
-- With security_invoker=true, the querying role's own RLS context applies,
-- same as if it queried the base tables directly.
--
-- Never references training_schedules (does not exist live / must not be
-- created). course_schedules is canonical.
...
existing_cert as (
  -- distinct on picks the most recent non-revoked, non-deleted certificate
  -- per pair; the partial unique index (certificates_active_schedule_participant_uniq)
  -- guarantees at most one such row per pair going forward, this is just
  -- defensive against any pre-index edge case.
