...
export default function ServicesPage() {
  return (
    <main className="services-page">
      ...
      <section className="service-detail-section"><div className="container">{serviceDetails.map(([id, title, text, tags, image], index) => <article id={id} className={`service-detail ${index % 2 ? "service-detail-reverse" : ""}`} key={id}><div className="service-detail-copy"><span className="eyebrow">0{index + 1} &middot; {title}</span><h2>{title}</h2><p>{text}</p><ul>{tags.map((tag) => <li key={tag}>{tag}</li>)}</ul><div className="service-detail-outcome"><small>BUSINESS OUTCOME</small><strong>{serviceOutcomes[id]}</strong></div><a className="btn btn-primary" href="/#contact">Discuss This Service</a></div><figure className="service-detail-media"><Image src={image} alt={`${title} industrial training and consultancy visual.`} width={1100} height={760} sizes="(max-width: 920px) 100vw, 50vw" /></figure></article>)}</div></section>
