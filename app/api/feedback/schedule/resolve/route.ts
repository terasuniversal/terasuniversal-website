...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await service.rpc("resolve_schedule_feedback_participant" as never, {
  ...
  if (error || !(data as any)?.[0]?.feedback_token) {
    if (error) console.error("schedule feedback lookup failed", { code: error.code });
    return NextResponse.json({ error: NEUTRAL_ERROR }, { status: 400 });
  }

  return NextResponse.json({ redirectTo: `/feedback/${(data as any)[0].feedback_token}` });
}
