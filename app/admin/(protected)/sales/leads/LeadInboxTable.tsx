        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.lead_metadata_id}>
              ...
              <td>
                <div>{r.email ?? "—"}</div>
                {r.phone && <div style={{ color: "var(--ta-muted)", fontSize: 12.5 }}>{r.phone}</div>}
              </td>
              <td>{r.subject ?? "—"}</td>
              <td><Badge status={r.status} /></td>
              <td>{r.assigned_to ? staffNames.get(r.assigned_to) ?? "—" : <span style={{ color: "var(--ta-muted)" }}>Unassigned</span>}</td>
              <td><FollowUpBadge state={followUpState(r.follow_up_at, r.status)} /></td>
