...
import { PageHead, Card, EmptyState, ErrorState, Pagination } from "../../../../../components/admin/ui";
...
export default async function SalesLeadsPage({
  ...
  let qualificationQueryError = null;
  if (hasQualificationFilter) {
    ...
    const { data: qualificationRows, error } = await qualificationQuery;
    qualificationQueryError = error;
  ...
  const { data: rows, count, error: rowsError } = qualificationIds?.length === 0 ? { data: [], count: 0, error: null } : await query;
  const leadRows = (rows ?? []) as SalesLeadInboxRow[];
  const leadIds = leadRows.map((lead) => lead.lead_metadata_id);
  const { data: qualificationRows, error: pageQualificationError } = leadIds.length ? await supabase.from("sales_lead_metadata").select("id, qualification_status, temperature, qualification_reason, disqualification_reason, created_at").in("id", leadIds) : { data: [], error: null };
  ...
  const { data: activityRows, error: activityError } = leadIds.length ? await supabase.from("sales_activity").select("lead_metadata_id, created_at").in("lead_metadata_id", leadIds).order("created_at", { ascending: false }) : { data: [], error: null };
  ...
  const { data: taskRows, error: taskError } = leadIds.length
    ? await supabase
        .from("sales_tasks")
  ...
  const queryError = staffError || qualificationQueryError || rowsError || pageQualificationError || activityError || taskError;
  ...
  return (
    <>
      ...
      >
        {queryError ? <ErrorState action={<a href="/admin/sales/leads">Reload leads</a>} /> : rows && rows.length > 0 ? (
