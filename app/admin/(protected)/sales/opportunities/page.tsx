...
import { PageHead, Card, EmptyState, ErrorState, Pagination, StatCard } from "../../../../../components/admin/ui";
import {
...
async function loadSignals(supabase: any, opportunityIds: string[]) {
  if (!opportunityIds.length) return { tasksByOpportunity: new Map<string, SalesTaskRow[]>(), activityByOpportunity: new Map<string, ActivitySignal[]>(), error: null };
  const [{ data: taskRows, error: taskError }, { data: activityRows, error: activityError }] = await Promise.all([
  ...
  return { tasksByOpportunity, activityByOpportunity, error: taskError || activityError };
}
...
export default async function OpportunitiesPage({
  ...
  const [{ data: rawRows, count, error: rowsError }, { data: summaryRaw, error: summaryError }] = await Promise.all([
    query,
  ...
  const queryError = staffError || rowsError || summaryError || signals.error;
  ...
  return (
    <>
      ...
      <Card>
        {queryError ? <ErrorState action={<a href="/admin/sales/opportunities">Reload opportunities</a>} /> : rows.length > 0 ? <OpportunityTable rows={rows} staffNames={staffNames} /> : <EmptyState icon="🎯" message={filtersApplied ? "No opportunities match these filters." : "No opportunities yet. Convert a qualified lead from Lead Detail to create one."} />}
      </Card>
