  ...
  return (
    <>
      ...
      <ul className="ta-lead-cards">
        {rows.map((r) => (
          <li className="ta-card ta-lead-card" key={r.id}>
            <div className="ta-lead-card-top">
              <code style={{ fontSize: 12 }}>{r.opportunity_no}</code>
            ...
            <div className="ta-lead-card-grid">
              ...
              <span>{r.expected_close_date ? new Date(r.expected_close_date).toLocaleDateString("en-MY", { day: "numeric", month: "short", year: "numeric" }) : "—"}</span>
              <span>Value</span>
              <span>{r.estimated_value != null ? `RM ${Number(r.estimated_value).toLocaleString("en-MY", { minimumFractionDigits: 2 })}` : "—"}</span>
            </div>
            <div className="ta-lead-card-action">
              ...
              <Link href={`/admin/sales/opportunities/${r.id}`} className="ta-btn ta-btn-outline ta-btn-sm">View opportunity →</Link>
            </div>
          </li>
        ))}
      </ul>
