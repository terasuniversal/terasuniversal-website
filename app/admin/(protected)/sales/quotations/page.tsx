...
import { PageHead, Card, Badge, EmptyState, ErrorState, Pagination } from "../../../../../components/admin/ui";
...
export default async function QuotationsPage({
  ...
  const [{ data: taskRows, error: taskError }, { data: invoiceRows, error: invoiceError }] = quotationIds.length > 0
    ? await Promise.all([
  ...
  const { data: paymentRows, error: paymentError } = invoiceIds.length > 0
  ...
  return (
    <>
      ...
      <Card>
        {rowsError || taskError || invoiceError || paymentError ? <ErrorState action={<a href="/admin/sales/quotations">Reload quotations</a>} /> : rows && rows.length > 0 ? (
          <>
