...
export default async function QuotationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const trainingDetailsResult = quotationTrainingDetailsSchema.safeParse(q.training_details);
  const trainingDetails = trainingDetailsResult.success ? trainingDetailsResult.data : undefined;
    ...
    <>
      ...
      <div className={`ta-lead-detail-grid quotation-editor-layout${q.status === "draft" && canManage ? " is-editing" : ""}`} style={{ marginTop: 16 }}>
        <div>
          ...
          {!(q.status === "draft" && canManage) && trainingDetails && <Card title="Training Snapshot">
            <div className="ta-card-pad" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
              <div><div style={{ color: "var(--ta-muted)", fontSize: 12 }}>Programme</div><div>{trainingDetails.programme.course_name_snapshot || "—"}</div></div>
              <div><div style={{ color: "var(--ta-muted)", fontSize: 12 }}>Dates / Duration</div><div>{trainingDetails.programme.start_date || "—"} → {trainingDetails.programme.end_date || "—"} · {trainingDetails.programme.duration_label || "—"}</div></div>
              <div><div style={{ color: "var(--ta-muted)", fontSize: 12 }}>Venue</div><div>{trainingDetails.venue.type === "teras_hq" ? "TERAS HQ" : "In-House"} · {trainingDetails.venue.name || "—"}</div></div>
              <div><div style={{ color: "var(--ta-muted)", fontSize: 12 }}>Participants</div><div>{trainingDetails.participants.count} Pax · {trainingDetails.participants.tbc || trainingDetails.participants.names.length === 0 ? "TBC" : trainingDetails.participants.names.join(", ")}</div></div>
              <div style={{ gridColumn: "1 / -1" }}><div style={{ color: "var(--ta-muted)", fontSize: 12 }}>Inclusions</div><div>{[trainingDetails.accommodation.included ? `Accommodation${trainingDetails.accommodation.nights ? ` (${trainingDetails.accommodation.nights} nights)` : ""}` : null, trainingDetails.meals.included ? `Meals${trainingDetails.meals.meals_per_day ? ` (${trainingDetails.meals.meals_per_day}/day)` : ""}` : null, trainingDetails.inclusions.training_notes ? "Training notes" : null, trainingDetails.inclusions.practical_assessment ? "Practical assessment" : null, trainingDetails.inclusions.certificate ? "Certificate" : null, ...trainingDetails.inclusions.other].filter(Boolean).join(", ") || "—"}</div></div>
            </div>
          </Card>}
