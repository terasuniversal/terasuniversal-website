...
async function save(id: string | null, _p: GalleryFormState, form: FormData): Promise<GalleryFormState> {
  await requireRole("editor"); await requireModuleAccess("gallery"); const image_url = String(form.get("image_url") ?? "").trim();
...
export async function archiveGalleryItem(id: string) { await requireRole("admin"); await requireModuleAccess("gallery"); const s = await createSupabaseServerClient(); const { error } = await (s.from("gallery_images") as any).update({ deleted_at: new Date().toISOString() }).eq("id", id); if (error) throw new Error("Unable to archive image."); revalidatePath("/admin/gallery"); }
