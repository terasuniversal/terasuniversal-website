...
export default async function UsersPage({
  ...
  let query = supabase
    .from("profiles")
    .select("id, email, full_name, job_title, department, role, is_active, access_control_enabled, last_login_at, created_at", { count: "exact" })
  ...
  const users = (data ?? []) as Pick<Profile, "id" | "email" | "full_name" | "job_title" | "department" | "role" | "is_active" | "access_control_enabled" | "last_login_at" | "created_at">[];
  const { data: accessRows } = users.length
    ? await supabase.from("staff_module_access").select("user_id, module_key").in("user_id", users.map((user) => user.id))
    : { data: [] as { user_id: string; module_key: string }[] };
  const accessCount = new Map<string, number>();
  for (const row of (accessRows ?? []) as { user_id: string }[]) accessCount.set(row.user_id, (accessCount.get(row.user_id) ?? 0) + 1);
  ...
  return <>
    <PageHead title="Staff Users" subtitle="Staff directory, department and explicit module access." action={<div style={{ display: "flex", gap: 8 }}><Link href="/admin/audit" className="ta-btn ta-btn-outline">View audit log</Link><Link href="/admin/users/new" className="ta-btn ta-btn-primary">+ Add Staff</Link></div>} />
    ...
    <Card title="Staff accounts">
      {users.length ? <div className="ta-table-wrap"><table className="ta-table"><thead><tr><th>Staff member</th><th>Department</th><th>Role</th><th>Access</th><th>Status</th><th>Actions</th></tr></thead><tbody>
        {users.map((user) => <tr key={user.id}><td><strong>{user.full_name || "Unnamed user"}</strong><div style={{ color: "var(--ta-muted)", fontSize: 12 }}>{user.email}{user.job_title ? ` · ${user.job_title}` : ""}</div></td><td>{user.department ? user.department.replace("_", " ") : <span style={{ color: "var(--ta-muted)" }}>—</span>}</td><td><Badge status={user.role} /></td><td>{user.role === "super_admin" ? "Unrestricted" : `${accessCount.get(user.id) ?? 0} modules`}</td><td><Badge status={user.is_active ? "active" : "inactive"} /></td><td><Link className="ta-btn ta-btn-outline ta-btn-sm" href={`/admin/users/${user.id}`}>View/Edit</Link></td></tr>)}
    ...
    <p className="ta-page-note">Super Admin only. Passwords are never displayed or stored in the CMS; Supabase Auth handles invitation and reset flows.</p>
