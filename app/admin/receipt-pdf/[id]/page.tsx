...
export default async function ReceiptPdfPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="receipt-print-root"><style>{`
    ...
    .receipt-print-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.receipt-print-card{padding:10px 12px;border:1px solid #E3E9F0;line-height:1.45;overflow-wrap:anywhere}.receipt-print-card strong{color:#0B3A63}.receipt-print-value{white-space:pre-wrap;overflow-wrap:anywhere}
  ...
  `}</style>{Array.from({ length: pageCount }, (_, pageIndex) => <main className="receipt-print-paper" key={pageIndex}>
    ...
    {pageIndex === 0 ? <>
      ...
      <section className="receipt-print-section"><h2 className="receipt-print-title">Payment Details</h2><div className="receipt-print-grid"><div className="receipt-print-card"><div><strong>Payment Method</strong></div><div>{paymentMethodLabel(providerLabel, row.payment_method_snapshot)}</div></div><div className="receipt-print-card"><div><strong>Payment Date</strong></div><div>{fmtDate(row.receipt_date)}</div></div><div className="receipt-print-card"><div><strong>Payment Reference</strong></div><div className="receipt-print-value">{row.payment_reference_snapshot || "—"}</div></div>{row.provider_reference_snapshot && <div className="receipt-print-card"><div><strong>Provider Reference</strong></div><div className="receipt-print-value">{row.provider_reference_snapshot}</div></div>}</div></section>
